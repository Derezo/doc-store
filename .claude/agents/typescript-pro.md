...
### Monorepo Structure
- **packages/shared** — Zod schemas + TypeScript types consumed by both api and web
- **packages/api** — Express 5 REST API + WebDAV server (ESM, `"type": "module"`)
- **packages/web** — Next.js 15 frontend with React 19
- Shared package must be built before other packages (`npm run build` handles ordering)

### ESM Requirements (Critical)
- All API imports require `.js` extensions: `import { config } from './config.js'`
- `"type": "module"` in packages/api and packages/shared
- When moving/renaming files, ALL import paths must be updated with `.js` extensions

### TypeScript Configuration
- Root `tsconfig.json` at project root
- Per-package configs: `packages/shared/tsconfig.json`, `packages/api/tsconfig.json`, `packages/web/tsconfig.json`
- API targets ES2022, uses Node16 module resolution

### Key Type Patterns
- **Zod schemas** in `packages/shared/src/` — auth, vault, document, api-key, search schemas
- **Drizzle custom types**: `customType` for PostgreSQL `tsvector` columns in `packages/api/src/db/schema.ts`
- **Express 5 wildcards**: `/*splat` syntax — captured as `req.params.splat` (may be array, not string)
- **Custom error hierarchy**: `AppError` base class in `packages/api/src/utils/errors.ts` with subclasses: `ValidationError` (400), `AuthenticationError` (401), `AuthorizationError` (403), `NotFoundError` (404), `ConflictError` (409), `RateLimitError` (429)

### Key Files
- `packages/api/src/db/schema.ts` — 7 tables: users, invitations, sessions, vaults, documents, document_versions, api_keys
- `packages/api/src/config.ts` — Zod-validated environment config with development defaults
- `packages/api/src/utils/errors.ts` — Custom error class hierarchy
- `packages/api/src/utils/jwt.ts` — JWT handling with jose library (HS256)
- `packages/shared/src/` — All shared Zod schemas and derived TypeScript types
