...
#### WebDAV Handler
- `packages/api/src/webdav/index.ts` — Single large file handling all WebDAV methods

#### Shared Schemas
- `packages/shared/src/` — Zod schemas exported to both API and web
- Changes here affect both packages — coordinate carefully

### ESM Import Extensions (Critical During Refactoring)
When moving or renaming any `.ts` file in packages/api or packages/shared:
- ALL import paths must use `.js` extensions (ESM requirement)
- Example: `import { foo } from './bar.js'` (even though source file is `bar.ts`)
- Search for all imports of the old path and update them

### Drizzle Schema Location
- `packages/api/src/db/schema.ts` — Single file with all 7 table definitions
- Schema changes require migration: `npx drizzle-kit generate` then `npx drizzle-kit migrate`

### Shared Package Build Dependency
- `packages/shared` must be built before `packages/api` and `packages/web` can consume it
- After refactoring shared schemas, run `npm run build` from root to verify

### No Existing Tests (Refactoring Risk)
- There are NO automated tests in this project
- Refactoring must be done with extra caution — no safety net
- Verify behavior manually after each change
- Consider writing characterization tests before major refactors

### Error Hierarchy
- `AppError` → `ValidationError` (400), `AuthenticationError` (401), `AuthorizationError` (403), `NotFoundError` (404), `ConflictError` (409), `RateLimitError` (429)
- Located at `packages/api/src/utils/errors.ts`
- Used consistently across all services and routes

### Middleware Stack (Order Matters)
Refactoring middleware requires understanding the mount order:
1. Security headers + request logger
2. WebDAV (`/webdav`) — BEFORE express.json()
3. express.json() + cookieParser()
4. API rate limiter
5. Auth routes (stricter rate limit)
6. Resource routes
7. Global error handler (must remain last)
