...
- Catch-all route: `(app)/vaults/[vaultId]/[...path]/page.tsx` for document paths

### Editor System (Three Modes)
- **Tiptap** — WYSIWYG rich text editing
- **CodeMirror 6** — Source markdown editing
- **react-markdown** — Read-only view mode
- Editor state managed by Zustand store at `packages/web/src/lib/stores/editor.store.ts`

### State Management (Zustand)
- `packages/web/src/lib/stores/auth.store.ts` — Authentication state + tokens
- `packages/web/src/lib/stores/vault.store.ts` — Active vault selection
- `packages/web/src/lib/stores/editor.store.ts` — Editor mode + content state

### API Client
- `packages/web/src/lib/api-client.ts` — `ky` HTTP client
- Auto-injects Bearer token from auth store via `beforeRequest` hook
- Handles 401 with automatic token refresh via `beforeRetry` hook
- Credentials: 'include' (for httpOnly refresh token cookie)

### Styling (Tailwind v4 — IMPORTANT)
- Uses CSS-based config (`@import "tailwindcss"` in CSS files)
- Does NOT use `tailwind.config.js` — this is Tailwind v4 syntax
- Theme customization is done in CSS, not JavaScript config

### Markdown Rendering Pipeline
- `packages/web/src/components/viewer/MarkdownViewer.tsx`
- Rehype chain: `rehype-raw` → `rehype-sanitize` → `rehype-highlight` → `rehype-katex`
- CRITICAL: `rehype-sanitize` MUST follow `rehype-raw` to prevent XSS
- Custom plugins: `remark-wikilinks` (Obsidian `[[links]]`), callout blocks

### Key Directories
- `packages/web/src/app/(auth)/` — Login, register pages
- `packages/web/src/app/(app)/` — Protected app pages (vaults, search, settings, admin)
- `packages/web/src/components/` — Shared components including editor and viewer
- `packages/web/src/lib/` — API client, stores, markdown utilities
