...
### Express 5 Middleware Mount Order (Critical)
The order matters — WebDAV routes are registered BEFORE `express.json()`:
1. Security headers + request logger
2. **WebDAV** (`/webdav`) with its own rate limiter — raw body streaming
...
- **JWT**: jose library, HS256, 15-min access token + httpOnly refresh cookie
- **API Keys**: `ds_k_` prefix, looked up by prefix for fast DB lookup, scope-restricted (`read`/`write`)
...
- **Chokidar watcher**: debounce 500ms, ignores `.obsidian/` and non-`.md` files
...
- Sync service: `packages/api/src/services/sync.service.ts`
- Filesystem service: `packages/api/src/services/filesystem.service.ts`

### WebDAV
- Hand-implemented (NOT nephele) for Express 5 compatibility
- Methods: OPTIONS, GET, HEAD, PUT, DELETE, MKCOL, MOVE, COPY, PROPFIND, LOCK, UNLOCK
- CORS restricted to Obsidian origins only (`app://obsidian.md`, `capacitor://localhost`, `http://localhost`)
- Handler: `packages/api/src/webdav/index.ts`
- Auth: `packages/api/src/webdav/authenticator.ts`

### Express 5 Gotchas
- Wildcard routes use `/*splat` syntax, captured as `req.params.splat` (may be array)
- ESM imports require `.js` extensions everywhere
- Route prefix: `API_PREFIX` = `/api/v1` from shared constants

### Key Service Files
- `packages/api/src/services/auth.service.ts` — Login, register, token refresh
- `packages/api/src/services/document.service.ts` — CRUD operations
- `packages/api/src/services/vault.service.ts` — Vault management
- `packages/api/src/services/search.service.ts` — Full-text search
- `packages/api/src/services/sync.service.ts` — File watcher + reconciliation
- `packages/api/src/services/filesystem.service.ts` — Atomic file I/O
- `packages/api/src/services/api-key.service.ts` — API key management
- `packages/api/src/services/invitation.service.ts` — User invitations
- `packages/api/src/services/email.service.ts` — Email sending

### Route Files
- `packages/api/src/routes/auth.routes.ts`
- `packages/api/src/routes/documents.routes.ts`
- `packages/api/src/routes/vaults.routes.ts`
- `packages/api/src/routes/search.routes.ts`
- `packages/api/src/routes/api-keys.routes.ts`
