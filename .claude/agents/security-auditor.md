...
#### WebDAV (`packages/api/src/webdav/`)
- Exposed to Obsidian clients over HTTP — handles raw file upload/download
- CORS must be restricted to Obsidian origins only (`app://obsidian.md`, `capacitor://localhost`)
- LOCK/UNLOCK operations — verify proper cleanup and timeout
- MOVE/COPY — must update ALL child files in DB, not just parent directory
- Path traversal risk in file operations — verify path sanitization

#### REST API (`packages/api/src/routes/`)
- JWT auth (15-min expiry) + API key auth (`ds_k_` prefix)
- Refresh token stored as httpOnly cookie — requires CSRF protection via `X-Requested-With` header
- Rate limiting on login/register endpoints
- Document paths via wildcard `/*splat` — verify no path injection

#### Markdown Rendering (`packages/web/src/components/viewer/`)
- `rehype-raw` allows arbitrary HTML in Markdown — XSS vector
- `rehype-sanitize` MUST follow `rehype-raw` in the pipeline
- `dangerouslySetInnerHTML` usage — must escape first, restore safe tags
- Custom remark plugins (wikilinks, callouts) — verify no injection

#### Database (`packages/api/src/services/search.service.ts`)
- SQL LIKE patterns — `%` and `_` must be escaped to prevent wildcard injection
- `websearch_to_tsquery` — verify parameterized queries
- Check-then-insert patterns (registration) — need transaction + `FOR UPDATE`

#### Filesystem (`packages/api/src/services/filesystem.service.ts`)
- File paths constructed from user input (vault slug, document path)
- Atomic writes (temp + rename) — verify no TOCTOU race
- Chokidar watcher — verify no symlink following

### Security Patterns Already Implemented
- argon2 password hashing (`packages/api/src/services/auth.service.ts`)
- JWT via jose library with HS256 (`packages/api/src/utils/jwt.ts`)
- API key prefix lookup for fast verification without exposing secrets
- Environment validation via Zod on startup (`packages/api/src/config.ts`)
- Security headers middleware (`packages/api/src/middleware/security-headers.ts`)
- Request validation middleware (`packages/api/src/middleware/validate.ts`)

### Priority Audit Files
1. `packages/api/src/middleware/auth.ts` — Auth bypass vectors
2. `packages/api/src/webdav/index.ts` — WebDAV handler (largest attack surface)
3. `packages/api/src/webdav/authenticator.ts` — WebDAV auth
4. `packages/api/src/services/search.service.ts` — SQL query construction
5. `packages/web/src/components/viewer/MarkdownViewer.tsx` — XSS via Markdown
6. `packages/api/src/services/filesystem.service.ts` — Path traversal
7. `packages/api/src/routes/auth.routes.ts` — Login/register flows
8. `packages/api/src/middleware/security-headers.ts` — CORS/CSP configuration
