...
### Database Setup
- **PostgreSQL 17** via Docker Compose (`docker compose up -d`)
...
|-------|-------------|-------|
| `users` | id (UUID PK), email, password_hash, name, role | argon2 password hashes |
| `invitations` | id, email, token, expires_at | Invitation-only registration |
| `sessions` | id, user_id, refresh_token | JWT refresh token storage |
| `vaults` | id, user_id, name, slug | Markdown document containers |
| `documents` | id, vault_id, path, title, content_search (tsvector) | Full-text search column |
| `document_versions` | id, document_id, content, created_at | Version history |
| `api_keys` | id, user_id, prefix, hashed_key, scopes, vault_id | `ds_k_` prefix, scope-restricted |

### Full-Text Search Implementation
- Custom Drizzle type: `customType` for `tsvector` columns
- Search function: `websearch_to_tsquery('english', query)` — natural language syntax
- Ranking: `ts_rank_cd(content_search, query)` for relevance ordering
- Snippets: `ts_headline('english', content, query)` for result previews
- GIN index on `content_search` column for fast lookups
- Search service: `packages/api/src/services/search.service.ts`
- Backfill CLI: `npm run backfill-search -w packages/api`

### SQL LIKE Escaping (Security)
- `%` and `_` characters MUST be escaped in LIKE patterns to prevent wildcard injection
- Relevant in document path queries and search filters

### Drizzle Migration Workflow
1. Edit `packages/api/src/db/schema.ts`
2. `cd packages/api && npx drizzle-kit generate` — generates SQL migration
3. `cd packages/api && npx drizzle-kit migrate` — applies migration
4. NEVER edit generated SQL migration files directly

### UUID Primary Keys
- All tables use UUID primary keys (`uuid` type with `gen_random_uuid()` default)
- Foreign keys reference UUIDs, not integers

### Array Columns
- `api_keys.scopes` — text array for permission scopes (`read`, `write`)
