...
### Security Checklist (Must-Check Items)
These are documented security lessons from this project. Every review MUST check:

1. **rehype-sanitize after rehype-raw**: In `packages/web/src/components/viewer/MarkdownViewer.tsx`, `rehype-sanitize` MUST follow `rehype-raw` to block script injection via Markdown HTML
2. **SQL LIKE escaping**: Any LIKE query must escape `%` and `_` to prevent wildcard injection
3. **Registration race condition**: User registration with check-then-insert needs DB transaction + `FOR UPDATE` to prevent duplicate accounts
4. **WebDAV CORS**: Must only allow known Obsidian origins (`app://obsidian.md`, `capacitor://localhost`), NOT wildcard `*`
5. **CSRF protection**: Cookie-based endpoints (refresh) must require `X-Requested-With` header
6. **Token exposure**: API key secret tokens must NEVER appear in list responses — only returned on creation
7. **Directory MOVE/COPY sync**: WebDAV directory MOVE/COPY operations must sync ALL child files to DB, not just the directory
8. **dangerouslySetInnerHTML**: Must escape HTML first, then selectively restore safe tags

### Framework-Specific Review Points
- **Express 5 wildcards**: Verify `/*splat` syntax, check that `req.params.splat` is handled as potentially an array
- **ESM imports**: All `.ts` imports in packages/api must use `.js` extension
- **Tailwind v4**: CSS-based config — reject any `tailwind.config.js` additions
- **Atomic file writes**: File writes must use temp file + rename pattern

### No Test Framework Warning
This project has NO test framework configured. When reviewing code:
- Flag any changes to critical paths (auth, search SQL, file sync) that lack tests
- Note this as technical debt in review feedback
- Recommend Vitest for API/shared, @testing-library/react for web

### Key Files for Review Focus
- `packages/api/src/middleware/auth.ts` — Authentication middleware
- `packages/api/src/webdav/` — WebDAV implementation
- `packages/api/src/services/search.service.ts` — SQL query construction
- `packages/web/src/components/viewer/MarkdownViewer.tsx` — Markdown rendering
- `packages/api/src/services/sync.service.ts` — File watcher with concurrency
- `packages/api/src/utils/errors.ts` — Error hierarchy (AppError → ValidationError, AuthenticationError, etc.)
